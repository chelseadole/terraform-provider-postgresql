...
func TestConfigConnParams(t *testing.T) {
	var tests = []struct {
		...
		{&Config{BinaryParameters: true}, []string{"binary_parameters=yes"}},
		{&Config{BinaryParameters: false, ExpectedVersion: semver.MustParse("14.2.0"), ApplicationName: "Terraform provider"}, []string{"fallback_application_name=Terraform+provider"}},
...
func TestConfigConnStr(t *testing.T) {
	var tests = []struct {
		...
		{&Config{Scheme: "postgres", Host: "localhost", Port: 5432, Username: "spaced user", Password: "spaced password", SSLMode: "disable", BinaryParameters: true}, "postgres://spaced%20user:spaced%20password@localhost:5432/postgres", []string{"connect_timeout=0", "binary_parameters=yes", "sslmode=disable"}},
	}
